...
identities with MSP *uisaMSP* and *attr1* OR with MSP *uisaMSP* and *attr2*.
...
with fabric-ca:
  ...
  1. When you register an identity, you can specify that an enrollment certificate
     ...
     *app1Admin* and *email*.
     ...
     ```bash
     fabric-ca-client register --id.name user1 --id.secret user1pw --id.type user --id.affiliation uisa --id.attrs 'app1Admin=true:ecert,email=user1@gmail.com'
     ```
